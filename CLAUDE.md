...
## Source-of-truth flow — data is ALREADY loaded

Flow: tool opens with inventory ALREADY aggregated from a synced data source (`data/inventory_source_final.csv` or `INVENTORY_SOURCE_URL`) — landing shows summary stats (#SKUs, #sources, last updated), never "please upload", never mentions Google Sheets ("synced data source"). Step 1 = optional override upload, CAPTURED ONLY (no parse/merge yet). Step 2+ = verified planning. Sheet has 3 sections: co-man | FST: | GR:. Co-man → buy-list (aggregate by code, Bazzini SUMIF, ingredient RM/IN feeds needs, packaging PK- kept-but-excluded, unmatched flagged); FST/GR → starting finished-goods inventory (On Hand drives calc, Available shown), kept separate. Formulas: units=pallets×minis_per_pallet; lbs=units×grams÷453.59237; need=Σ lbs×recipe%×(1+scrap); remaining=onhand+actuals−need+purchase. Constants (30,600 / 8 / 11 / recipe% / scrap%) editable before run; pallets never default to 1. 4 decisions suggested+overridable. Never blank.

Implementation: `lib/source.js` — `parseSourceCsv(text)` splits the 3 sections → `{coman, fst, gr, lastUpdated, perSourceUpdated, stats}`; `loadSource({url, csvText, fetchImpl})` tries the live URL then falls back to bundled (never blank); `captureOverride(state, fileRef)` stores the override unprocessed. `lib/engine.js` — `combineComan(rows, {ingredientMap})` sums by code / splits ingredient-vs-packaging / flags unmatched (same shape as `combineInventories`); `openingFinishedGoods(fstRows, grRows)` sums FST + GR On Hand & Available per base SKU (`stripBaseSku` strips -12/-MC6/…). UI (`public/index.html`): the CSV is inlined as `INVENTORY_SOURCE_CSV` (browser stand-in for the bundled file, kept in sync); `SRC=parseSourceCsv(...)` on load; step 0 = `renderSourceSummary()` + capture-only `#f_override`; `normalizeAll()` now combines from `SRC.coman`, opening from `openingFinishedGoods(SRC.fst,SRC.gr)` (`openingFG`); editable `CONSTS` (minisPerPallet/grams/minisPerPouch) feed the production formulas. Verified in `verify.mjs` section (L). Note: the demo's MC-mini pallet suggestion is still 0/covered (Modis 13,284 ≤ on-hand) and overridable — the FST/GR source carries bar SKUs, not the mini, so `openingFinished` falls back to the Total Inventory OH reference.
...
### Modis — the demand forecast input (upstream connection)
Modis = the demand forecast input (one row per retailer×SKU×month, rolling ~14 months; sum 'Forecasted Delivery' by SKU×Month). It connects upstream planning: demand − on-hand finished goods → production needed → pallets (suggested, overridable) → existing engine. Verified: MBMC4001 May demand = 13,284 (= the old hand-entered constant). Parse ALL months (sets up multi-month). Bridge finished SKU (MBMC4001) → WIP (MCM1001-WIP) via BOM so demand maps to production. Modis is additive — tool still works without it.

Implementation: `parseModis(grid)` in `lib/parsers.js` returns `{demandBySkuMonth, demandConfirmedBySkuMonth, months, skus, flags}` (locate columns by header name; flag SKUs whose demand is entirely Not Confirmed). `suggestPalletsFromDemand({sku, month, demandBySkuMonth, openingInv, refprod, bom})` in `lib/engine.js` computes `ceil(max(0, demand − on-hand) / minis_pallet)`, bridging the finished SKU → WIP via `bom` (finished→wip) to read `minis_pallet`; returns `{value, why}` (value null + flag if unbridgeable). UI: `suggestedPallets()` uses it when Modis carries the SKU/month (`PLAN_MONTH='2026-05'`, `MC_FINISHED_SKU='MBMC4001'`), else falls back to the months-of-supply `suggestPallets()`. Demo seed `raw_samples.modis` reproduces the verified May values. In the default demo the MC-mini suggestion is **0/covered** (on-hand 24,627 ≥ May demand 13,284) — honest and overridable; the locked 336,600 @ 11-pallet scenario is a manual override, unaffected. Verified in `verify.mjs` section (K).

### Multi-month rolling plan (headline feature)
Multi-month rolling plan: user picks start month + #months. For each month: opening = previous month's ending (month 1 = current FST+GR on-hand); net_demand = demand − opening; production suggested (pallets, overridable, re-rolls downstream); ending = opening + production − demand; show weeks-of-supply (ending ÷ (forward_demand/4.33)) and months-of-supply (ending ÷ forward_demand). Per-month engine unchanged (units=pallets×minis_per_pallet; lbs=units×grams÷453.59237; need=Σ lbs×recipe%×(1+scrap)). Buy-list per month + combined total; raw on-hand also carries forward. SKUs shown as 'CODE — Name' (from FST/GR description/ref). Unconf badges/warnings removed (Modis forecast is understood to be mostly unconfirmed). Single-month = numMonths 1.

Implementation: `rollMultiMonth({startMonth, numMonths, demandByMonth, openingFinishedGoods, openingRawMaterials, constants, overrides, needsForPallets})` in `lib/engine.js` is PURE — `needsForPallets(pallets)` injects the verified engine so the per-month math is unchanged; returns `{months:[{opening,demand,netDemand,suggestedPallets,pallets,overridden,receipts,ending,weeksOfSupply,monthsOfSupply,needs,buy,why}], combinedNeeds, combinedBuy}`. forward_demand = next month in the Modis horizon (else current). combinedBuy = max(0, Σ needs − openingRaw); per-month buys net against depleting raw and sum to combinedBuy. `skuLabel(code, names)` in `lib/source.js` → "CODE — Name". UI: s3 leads with the rolling timeline (`renderMultiMonth` — month-range picker, per-month override `mmOverrides` re-rolls, consolidated + per-month buy-lists); the verified single-month production/buy-list stays below (= month 1). Names from `SKU_NAMES` (built from `opening_inv` descriptions). Verified in `verify.mjs` section (M).
